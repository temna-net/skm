...
function validate(theform) {
	...
	else if(!theform.edittorrent && theform.torrent && theform.torrent.value==""){
		showDialog('您没有选择种子文件'); return false; 
	}
    else if(theform.douban){
        var douban_url=theform.douban.value;
        var reg = /movie\.douban\.com/i;            // i表示不区分大小写
        if(douban_url.search(reg)<0) {
            showDialog('请填写合法的豆瓣URL地址');
            return false;
        }
        reg = /(\d{7,8})/g;  
