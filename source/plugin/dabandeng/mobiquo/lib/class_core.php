...
class discuz_core {
	...
	function _init_env() {
		...
		if(!defined('DISCUZ_CORE_FUNCTION') && !@include('lib/function_core.php')) {
...
//		}
	...
	function _init_session() {
		...
		if($this->init_session)	{
			...
			if($this->session->get('groupid') == 6) {
				...
				get_error(lang('message', 'user_banned'));
			}
	...
	function _init_misc() {
		...
		if($this->init_user) {
			if($this->var['group'] && isset($this->var['group']['allowvisit']) && !$this->var['group']['allowvisit']) {
				if($this->var['uid']) {
					...
					get_error(lang('message', 'user_banned'));
				} elseif((!defined('ALLOWGUEST') || !ALLOWGUEST) && !in_array(CURSCRIPT, array('member', 'api')) && !$this->var['inajax'] && !in_array($GLOBALS['request_method'], array('get_config', 'login'))) {
		...
		if($this->var['setting']['bbclosed']) {
			...
			} elseif(in_array($GLOBALS['request_method'], array('get_config', 'login')) || in_array(CURSCRIPT, array('admin', 'member', 'api')) || defined('ALLOWGUEST') && ALLOWGUEST) {
			} else {
