...
require_once ('include/function_discuzcode.php');
...
if($rushreply) {
	...
	if(TIMESTAMP > $rushresult['starttimefrom'] && TIMESTAMP < $rushresult['starttimeto'] && $_G['forum_thread']['closed']) {
		DB::query("UPDATE ".DB::table('forum_thread')." SET closed='0' WHERE tid='$_G[tid]'");
	} elseif((TIMESTAMP < $rushresult['starttimefrom'] || ($rushresult['starttimeto'] && TIMESTAMP > $rushresult['starttimeto']) || ($rushresult['stopfloor'] && $_G['forum_thread']['replies'] + 1 >= $rushresult['stopfloor'])) && $_G['forum_thread']['closed'] == 0) {
...
if(empty($_G['gp_viewpid'])) {
	...
	if($_G['forum_pagebydesc']) {
		...
		$_G['setting']['postperpage'] = $request_params[2] + 1;
		if($ordertype != 1) {
			$pageadd = "ORDER BY p.dateline LIMIT $start_limit, ".$_G['setting']['postperpage'];
		} else {
			$_G['forum_numpost'] = $_G['forum_thread']['replies'] + 2 - $_G['forum_numpost'] + ($page > 1 ? 1 : 0);
			$pageadd = "ORDER BY p.first DESC, p.dateline DESC LIMIT $start_limit, ".$_G['setting']['postperpage'];
		}
	}
...
if($_G['setting']['portalstatus']) {
	$allowpostarticle = $_G['group']['allowmanagearticle'] || getstatus($_G['member']['allowadmincp'], 2) || getstatus($_G['member']['allowadmincp'], 3);
...
function viewthread_procpost($post, $lastvisit, $ordertype, $special = 0) {
	...
	if(!defined('IN_ARCHIVER')) {
		$post['message'] = discuzcode($post['message'], $post['smileyoff'], $post['bbcodeoff'], $post['htmlon'] & 1, $_G['forum']['allowsmilies'], $_G['forum']['allowbbcode'], ($_G['forum']['allowimgcode'] && $_G['setting']['showimages'] ? 1 : 0), $_G['forum']['allowhtml'], 0, 0, $post['authorid'], $_G['cache']['usergroups'][$post['groupid']]['allowmediacode'] && $_G['forum']['allowmediacode'], $post['pid']);
		if($post['first']) {
