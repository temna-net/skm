        ...
        switch ($id) {
            ...
            default: 
                ...
                foreach($block_data['itemlist'] as $data)
                {
                    ...
                    $record_string = $data['id'].'|'.base64_encode(basic_clean($data['fields']['fulltitle'])).'|'.base64_encode(basic_clean($data['fields']['author'])).'|'.format_time(mobiquo_iso8601_encode($time));
                
                    $xmlrpc_record = new xmlrpcval($record_string, 'string');
