...
if($_GET['op'] == 'delete') {
    ...
    $idtype = ($type == 'forum' ? 'fid' : 'tid');
    $thevalue = DB::fetch_first('SELECT * FROM '.DB::table('home_favorite')." WHERE idtype='$idtype' AND id='$id' and uid='$_G[uid]'");
    if(empty($thevalue) || $thevalue['uid'] != $_G['uid']) {
        showmessage('favorite_does_not_exist');
    }

    if(submitcheck('deletesubmit')) {
        DB::query('DELETE FROM '.DB::table('home_favorite')." WHERE idtype='$idtype' AND id='$id' and uid='$_G[uid]'");
