...
if(!$_G['gp_submit']) {
	...
	showformfooter();
	showtablefooter();
} elseif($_G['gp_imchoice'] && $_G['gp_submit'] && $_G['adminid']=='1' && $_G['gp_formhash']==FORMHASH) {
	if($_G['gp_imchoice'] == '3'){
	...
	}elseif($_G['gp_imchoice'] == '2'){
			if(!$_G['gp_signsett'] || !$_G['gp_signt'])cpmsg("{$lang[import_14]}", '');
			$query1 = DB::query("SHOW TABLES LIKE '{$_G[gp_signsett]}'");
			$query2 = DB::query("SHOW TABLES LIKE '{$_G[gp_signt]}'");
			if(DB::num_rows($query1) <= 0 || DB::num_rows($query2) <= 0)cpmsg("{$lang[import_13]}", '');
