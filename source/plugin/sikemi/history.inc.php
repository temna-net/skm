...
if($_G['adminid']!=1){
	...
	$query=DB::query("select a.*,b.subject from ".DB::table('xbtit_history')." a left join ".DB::table('forum_thread')." b on a.tid=b.tid where a.uid={$_G['uid']} limit $start_limit ,$ppp");
}
$i=0;
while($history=DB::fetch($query)){
	...
	$historys[$i]['subject']=$history['subject'];
