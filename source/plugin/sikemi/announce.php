...
//*积分小于0则不能下载*/
/*
$credits_query=$db->query("select credits from cdb_members where uid={$curuid['id']}");
$credits=mysql_fetch_assoc($credits_query);
if(!(is_numeric($credits['credits']) && $credits['credits']>-20)){
		show_error("您的积分过低，先去和觅友们聊天赚积分吧！");
}*/
//---*积分小于0则不能下载---*/
...
//获取种子类型，用于统计流量
$rstype=$db->query("SELECT highlight,displayorder FROM pre_forum_thread WHERE tid={$tid} LIMIT 1");
...
if (mysql_num_rows($resu)==0){
	$db->query("INSERT INTO {$tablepre}xbtit_history (uid,infohash,date,active,agent,makedate,tid) VALUES ($uid,'$info_hash',UNIX_TIMESTAMP(),'yes','$agent',UNIX_TIMESTAMP(),{$tid})");
}
