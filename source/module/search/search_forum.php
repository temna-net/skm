...
if(!submitcheck('searchsubmit', 1)) {
	...
	if(!empty($searchid)) {
		...
		$query = DB::query("SELECT * FROM ".DB::table('forum_thread')." LEFT JOIN ".DB::table('xbtit_files')." ON cdb_forum_thread.tid= cdb_xbtit_files.tid WHERE cdb_forum_thread.tid IN ($index[ids]) AND displayorder>='0' ORDER BY $orderby $ascdesc LIMIT $start_limit, $_G[tpp]");
		while($thread = DB::fetch($query)) {
			$thread['subject'] = bat_highlight($thread['subject'], $keyword);
			$thread['realtid'] = $thread['isgroup'] == 1 ? $thread['closed'] : $thread['tid'];
            $thread['seeds'] = $thread['seeds'];//sikemi
            $thread['leechers'] = $thread['leechers'];//sikemi
            $thread['finished'] = $thread['finished'];//sikemi
            $thread['size'] = sizecount($thread['size']);//sikemi
