...
if(!empty($_G['gp_orderby']) && in_array($_G['gp_orderby'], array('lastpost', 'dateline', 'replies', 'views', 'recommends', 'heats','leechers','seeds','finished','size'))) {
...
//此处加入审核中帖子类型-3，用于使用户能够看到自己发布的但是未审核帖子
$displayorderadd = !$filterbool && $stickycount ? 't.displayorder IN (0, 1)' : 't.displayorder IN (0, 1, 2, 3, 4)';
$displayorderadd = '(('.$displayorderadd.') OR (t.displayorder IN (-3, -2) AND (t.authorid='.$_G['uid'].' OR '.$_G['adminid'].'>0)))';

/*skm join一个表查询 start*/
$xbtfiles="";
if(preg_match('/sicau_pt_x2/',$_G['forum']['threadplugin'])){
	$xbtfiles="LEFT JOIN ".DB::table('xbtit_files')." AS f
			ON (t.tid=f.tid)";
}
/*skm end*/
...
if(($start_limit && $start_limit > $stickycount) || !$stickycount || $filterbool) {
	...
	$querysticky = '';
	$sql = "SELECT * FROM ".DB::table($threadtable)." t $indexadd $xbtfiles
		WHERE $fidsql $filteradd AND ($displayorderadd)
		ORDER BY t.displayorder DESC, $_G[gp_orderby] $_G[gp_ascdesc]
		LIMIT ".($filterbool ? $start_limit : $start_limit - $stickycount).", $_G[tpp]";

} else {
	$sql_s = "SELECT * FROM ".DB::table($threadtable)." t $xbtfiles
	...
	if($_G['tpp'] - $stickycount + $start_limit > 0) {
		$sql = "SELECT * FROM ".DB::table($threadtable)." t $xbtfiles
			WHERE $fidsql $filteradd AND ($displayorderadd)
			ORDER BY IF(displayorder>0,displayorder,0) DESC, $_G[gp_orderby] $_G[gp_ascdesc]
...
$thread_memcache = $discuz->mem->get(sha1($sql.$sql_s));
$thread_tmp = array();
$thread_fid_wlist = array(14);

if(is_array($thread_memcache) && !in_array($_G['fid'],$thread_fid_wlist)){
	echo("<!--use cache-->");
	$thread_tmp = $thread_memcache;
}else{
	if($sql_s)$querysticky = DB::query($sql_s);
	if($sql)$query = DB::query($sql);
	while(($querysticky && $thread = DB::fetch($querysticky,MYSQL_BOTH)) || ($query && $thread = DB::fetch($query,MYSQL_BOTH))) {
		$thread_tmp[] = $thread;
	}
	$discuz->mem->set(sha1($sql.$sql_s),$thread_tmp,30);
}
foreach($thread_tmp as $thread){
	/*临时补充数据到xbt_file表 starts*/
	if(!$thread['tid']){
		$thread['tid'] = $thread[0];
		DB::query("INSERT INTO  ".DB::table('xbtit_files')." (
		`info_hash` ,
		`tid` ,
		`filename` ,
		`url` ,
		...
		VALUES ('".sha1(time()*1000)."',".$thread['tid'].",'0','0','0000-00-00 00:00:00',0,'false',  '0',  '0',  '0',  '0',UNIX_TIMESTAMP()
		);");
		require_once libfile('class/error');
		discuz_error::write_error_log("fix xbt_file insert tid ".$thread['tid']);
	}
	/*判断 end*/
	/*skm start 组装一些数据*/
	$thread['size'] = sizecount($thread['size']);
	/*skm end*/
