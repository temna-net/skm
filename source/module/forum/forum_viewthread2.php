...
echo('memcache !1');
$discuz->mem->set('suna','shenmao',2);
echo($discuz->mem->get('suna'));
echo('memcache !2');
print_r($_G['cache']['plugin']['sicau_pt_x2']);
print_r(unserialize($_G['cache']['plugin']['sicau_pt_x2']['detault_content']));
if(empty($_G['gp_viewpid'])) {
	$sufix = '';
	if($_G['gp_from'] == 'portal') {
		...
		foreach($_G['cache']['custominfo']['setting'][$type] as $key => $order) {
			...
			if(substr($key, 0, 10) == 'extcredits') {
				...
				if(in_array($i,array(3,4))){
					$v = '<dt>'.($extcredit['img'] ? $extcredit['img'].' ' : '').$extcredit['title'].'</dt><dd>'.sizecount($post['extcredits'.$i]*1024*1024).'</dd>';
				}else{
				/*上传下载单位换算end*/
