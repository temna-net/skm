...
//判断是不是种子torrent
//$fileexts=explode('.',$attach['filename']);
//$fileext=$fileexts[count($fileexts)-1];
//开始处理old种子下载
if(preg_match("/\.torrent$/", $attach['filename'])){
	dheader('location:'.$_G['setting']['discuzurl'].'/plugin.php?id=sicau_pt_x2:download&tid='.$attach['tid']);
	die();
}
